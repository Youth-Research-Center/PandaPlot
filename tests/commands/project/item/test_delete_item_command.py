...
class TestDeleteItemCommandChartSeriesCascade:
    ...
    time. Exercises this through the generic Item.on_items_removed hook
    (Chart's implementation), not any Chart/Dataset-specific knowledge in
    DeleteItemCommand itself. Uses a real Project (not the mocked
    find_item/remove_item/add_item of the other tests' sample_project) so
    add/remove/find and the chart's own data_series list all behave for
    real."""
    ...
    def test_execute_leaves_unrelated_charts_untouched(self, mock_app_context, project_with_chart):
        ...
        assert other_chart.id not in command._snapshots
